List<dynamic> categories = [
  {
    "_id": "6537ece708ff5b7de97d0695",
    "title": "Fried Rice",
    "value": "fried_rice",
    "imageUrl":
        "https://houseofnasheats.com/wp-content/uploads/2023/01/Chicken-Fried-Rice-Recipe-Square-1-500x500.jpg",
    "createdAt": "2023-10-24T16:12:23.571Z",
    "updatedAt": "2023-10-24T16:12:23.571Z",
    "__v": 0
  },
  {
    "_id": "65310f3381e4d98d60b093c5",
    "title": "Curry",
    "value": "curry",
    "imageUrl":
        "https://shwetainthekitchen.com/wp-content/uploads/2023/06/veg-fried-rice.jpg",
    "__v": 0
  },
  {
    "_id": "6531206cbbe4998e90af3feb",
    "title": "Pizza",
    "value": "pizza",
    "imageUrl":
        "https://shwetainthekitchen.com/wp-content/uploads/2023/06/veg-fried-rice.jpg",
    "__v": 0
  },
  {
    "_id": "6531209dbbe4998e90af3fef",
    "title": "Pasta",
    "value": "pasta",
    "imageUrl":
        "https://shwetainthekitchen.com/wp-content/uploads/2023/06/veg-fried-rice.jpg",
    "__v": 0
  },
  {
    "_id": "653120babbe4998e90af3ff1",
    "title": "Beverages",
    "value": "beverages",
    "imageUrl":
        "https://shwetainthekitchen.com/wp-content/uploads/2023/06/veg-fried-rice.jpg",
    "__v": 0
  },
  {
    "_id": "65312084bbe4998e90af3fed",
    "title": "Burgers",
    "value": "burgers",
    "imageUrl":
        "https://shwetainthekitchen.com/wp-content/uploads/2023/06/veg-fried-rice.jpg",
    "__v": 0
  },
  {
    "_id": "65310efb81e4d98d60b093c3",
    "title": "Chicken",
    "value": "chicken",
    "imageUrl":
        "https://shwetainthekitchen.com/wp-content/uploads/2023/06/veg-fried-rice.jpg",
    "__v": 0
  },
  {
    "_id": "653120e1bbe4998e90af3ff3",
    "title": "More",
    "value": "more",
    "imageUrl":
        "https://shwetainthekitchen.com/wp-content/uploads/2023/06/veg-fried-rice.jpg",
    "__v": 0
  }
];

List<dynamic> restaurants = [
  {
    "_id": "66abc9634705a60d2ea9ab0c",
    "title": "Lapisara Eatery",
    "time": "6:00am - 9:00pm",
    "imageUrl":
        "https://content.jdmagicbox.com/comp/omkareshwar/z3/9999p7280.7280.220522005649.y3z3/catalogue/neelkanth-restaurant-omkareshwar-omkareshwar-h-o-omkareshwar-2r00x7gxgg.jpg",
    "owner": "fdfsdfsdfs",
    "code": "41007428",
    "logoUrl":
        "https://marketplace.canva.com/EAFpeiTrl4c/1/0/1600w/canva-abstract-chef-cooking-restaurant-free-logo-9Gfim1S8fHg.jpg",
    "rating": 5.0,
    "ratingCount": "6765",
    "coords": {
      "id": "2023",
      "latitude": 37.78792117665919,
      "longitude": -122.41325651079953,
      "address": "Cameroon, Yaounde, Nkoabang",
      "title": "Lapisara Eatery",
      "latitudeDelta": 0.0122,
      "longitudeDelta": 0.0221
    }
  },
  {
    "_id": "66abcaae4705a60d2ea9ab0e",
    "title": "AWAE King",
    "time": "6:00am - 9:00pm",
    "imageUrl":
        "https://thumbs.dreamstime.com/b/african-cook-tribal-restaurant-south-africa-25811815.jpg",
    "owner": "sjgdsjgfjshhjs",
    "code": "41007428",
    "logoUrl":
        "https://marketplace.canva.com/EAFzZd9frfk/1/0/1600w/canva-colorful-illustrative-restaurant-master-chef-logo-4rQv_oY-CF8.jpg",
    "rating": 5.0,
    "ratingCount": "3278",
    "coords": {
      "id": "2020",
      "latitude": 37.785925590588505,
      "longitude": -122.41007428687641,
      "address": "Cameroon, Yaounde, Awae Escalier",
      "title": "Awae King",
      "latitudeDelta": 0.0122,
      "longitudeDelta": 0.0221
    }
  },
  {
    "_id": "66abcb394705a60d2ea9ab10",
    "title": "KMER Foods",
    "time": "6:00am - 9:00pm",
    "imageUrl":
        "https://www.shutterstock.com/image-photo/closeup-african-food-during-buffet-600nw-2131009016.jpg",
    "owner": "fgdgdfgd",
    "code": "41007428",
    "logoUrl":
        "https://marketplace.canva.com/EAFwIrrpQxo/1/0/1600w/canva-brown-flat-illustrative-chef-restaurant-logo-pRMi3FdE_Yc.jpg",
    "rating": 5.0,
    "ratingCount": "5666",
    "coords": {
      "id": "2022",
      "latitude": 37.787503258917035,
      "longitude": -122.39854938269353,
      "address": "Cameroon, Yaounde, Essos",
      "title": "KMER Foods",
      "latitudeDelta": 0.0122,
      "longitudeDelta": 0.0221
    }
  },
  {
    "_id": "66abcbee4705a60d2ea9ab14",
    "title": "MaMa Africa ",
    "time": "6:00am - 9:00pm",
    "imageUrl":
        "https://ftcsfo3digitaloceanspaces.b-cdn.net/optimized/3X/a/6/a640131bfabab517e0fe2c9c7488e0780f03f06a_2_690x430.jpeg",
    "owner": "sjgdsjgfjshhjs",
    "code": "41007428",
    "logoUrl":
        "https://static.vecteezy.com/ti/vecteur-libre/t1/6247091-golden-chef-logo-design-vector-template-utilisable-pour-restaurant-logo-and-food-company-vectoriel.jpg",
    "rating": 5,
    "ratingCount": "3278",
    "coords": {
      "id": "2021",
      "latitude": 37.78557922976825,
      "longitude": -122.40722000299483,
      "address": "Cameroon, Yaounde, Bastos",
      "title": "MaMa Africa ",
      "latitudeDelta": 0.0122,
      "longitudeDelta": 0.0221
    }
  }
];

List<Map<String, dynamic>> foods = [
  {
    "_id": "65316968f94c6496dc84f3c1",
    "title": "Water Fufu and Eru",
    "foodTags": ["Cameroonian", "Traditional", "meal", "Mascarpone", "Red oil"],
    "foodType": ["meal"],
    "code": "41007428",
    "isAvailable": true,
    "restaurant": "6530ea6bc9e72013e5b6592d",
    "rating": 4.9,
    "ratingCount": "420",
    "description":
        "A classic Cameroonian meal originating from the northwest region of Cameroon.",
    "price": 2000,
    "additives": [
      {"id": 1, "title": "Water fufu", "price": "500"},
      {"id": 2, "title": "Kanda", "price": "300"},
      {"id": 3, "title": "Meat", "price": "400"},
      {"id": 4, "title": "Eru", "price": "500"},
    ],
    "imageUrl":
        "https://upload.wikimedia.org/wikipedia/commons/thumb/e/ef/Le_Eru%2C_un_plat_camerounais.jpg/1200px-Le_Eru%2C_un_plat_camerounais.jpg",
    "__v": 0,
    "category": "6531209dbbe4998e90af3fef",
    "time": "35 min"
  },
  {
    "_id": "653168e9f94c6496dc84f3bf",
    "title": "O'KOK",
    "foodTags": ["Cameroonian", "sweet", "okok"],
    "foodType": ["Main Course", "Lunch", null],
    "code": "41007428",
    "isAvailable": true,
    "restaurant": "6530ea6bc9e72013e5b6592d",
    "rating": 4.7,
    "ratingCount": "310",
    "description":
        "A traditional Cameroonian dish with sugar originatong fron the south of Cameroon.",
    "price": 1500,
    "additives": [
      {"id": 1, "title": "Cassava", "price": "500"},
      {"id": 2, "title": "Bobolo", "price": "500"},
      {"id": 3, "title": "sugar", "price": "250"},
      {"id": 4, "title": "okok", "price": "500"},
    ],
    "imageUrl": "https://i.ytimg.com/vi/rgokTlfOJjc/sddefault.jpg",
    "__v": 2,
    "Category": "6531209dbbe4998e90af3fef",
    "time": "20 min"
  },
  {
    "_id": "653169a9f94c6496dc84f3c3",
    "title": "Vegan Salad Bowl",
    "foodTags": ["Vegan", "Healthy", "Salad", "Fresh", "Organic"],
    "foodType": ["Starter", "Lunch", "Dinner", "Health", "Vegan"],
    "code": "41007428",
    "isAvailable": true,
    "restaurant": "6530ea6bc9e72013e5b6592d",
    "rating": 4.6,
    "ratingCount": "230",
    "description":
        "A refreshing mix of organic vegetables, nuts, seeds, and a tangy vinaigrette.",
    "price": 2500,
    "additives": [
      {"id": 1, "title": "Mixed Greens", "price": "500"},
      {"id": 2, "title": "Walnuts", "price": "500"},
      {"id": 3, "title": "Quinoa", "price": "550"},
      {"id": 4, "title": "Cherry Tomatoes", "price": "300"},
      {"id": 5, "title": "Vinaigrette", "price": "250"}
    ],
    "imageUrl":
        "https://cdn.loveandlemons.com/wp-content/uploads/2020/06/IMG_25456.jpg",
    "__v": 0,
    "Category": "6531209dbbe4998e90af3fef",
    "time": "55 min"
  },
  {
    "_id": "65316771f94c6496dc84f3bd",
    "title": "Margherita Pizza",
    "foodTags": ["Italian", "Cheesy", "Vegetarian"],
    "foodType": ["Main Course"],
    "code": "41007428",
    "isAvailable": true,
    "restaurant": "6530ebbcc9e72013e5b65933",
    "rating": 4.6,
    "ratingCount": "150",
    "description":
        "A classic Margherita pizza with fresh tomatoes, mozzarella cheese, basil, and olive oil.",
    "price": 3500,
    "additives": [
      {"id": 1, "title": "Cheese", "price": "500"},
      {"id": 2, "title": "Pepperoni", "price": "500"},
      {"id": 3, "title": "Ketch up", "price": "150"}
    ],
    "imageUrl":
        "https://i.nefisyemektarifleri.com/2017/01/25/pizza-margarita-margherita-italyan-hamur-10.jpeg",
    "__v": 0,
    "Category": "6531209dbbe4998e90af3fef",
    "time": "30 min"
  },
  {
    "_id": "65316a01f94c6496dc84f3c7",
    "title": "Tropical Fruit Smoothie",
    "foodTags": ["Fruit", "Smoothie", "Refreshing", "Sweet", "Cold"],
    "foodType": ["Drink", "Breakfast", "Snack", "Dessert", "Vegan"],
    "code": "41007428",
    "isAvailable": true,
    "restaurant": "6530ea6bc9e72013e5b6592d",
    "rating": 4.7,
    "ratingCount": "280",
    "description":
        "A delightful blend of tropical fruits, creating the perfect sweet and refreshing drink.",
    "price": 1500,
    "additives": [
      {"id": 1, "title": "Mango", "price": "600"},
      {"id": 2, "title": "Pineapple", "price": "300"},
      {"id": 3, "title": "Banana", "price": "300"},
      {"id": 4, "title": "Coconut Milk", "price": "300"},
      {"id": 5, "title": "Ice", "price": "500"}
    ],
    "imageUrl":
        "https://www.fannetasticfood.com/wp-content/uploads/2021/09/Berry-Yogurt-Smoothie-FF-featured-image-720x720.jpg",
    "__v": 0,
    "category": "6531209dbbe4998e90af3fef",
    "time": "25 min"
  },
  {
    "_id": "653169d8f94c6496dc84f3c5",
    "title": "Mixed Grill Platter",
    "foodTags": ["Barbecue", "Meat", "Grilled", "Spicy", "Savory"],
    "foodType": [
      "Main Course",
      "Dinner",
      "Grill",
      "Non-Vegetarian",
      "Barbecue"
    ],
    "code": "41007428",
    "isAvailable": true,
    "restaurant": "6530eb23c9e72013e5b6592f",
    "rating": 4.8,
    "ratingCount": "320",
    "description":
        "A succulent assortment of grilled meats, served with sides and sauces.",
    "price": 5500,
    "additives": [
      {"id": 1, "title": "Chicken", "price": "1500"},
      {"id": 2, "title": "Beef", "price": "1000"},
      {"id": 3, "title": "Lamb", "price": "1000"},
      {"id": 4, "title": "Pork", "price": "1500"},
      {"id": 5, "title": "Barbecue Sauce", "price": "700"}
    ],
    "imageUrl":
        "https://tandoorikabobgrill.com/wp-content/uploads/2022/04/arabic-mix-grilled.jpg",
    "__v": 0,
    "Category": "6531209dbbe4998e90af3fef",
    "time": "45 min"
  }
];

const cart = [
  {
    "_id": "653b6588541d2aa2c1e89cd1",
    "userId": "6537a4448cd1bd140ebddcee",
    "productId": {
      "_id": "65316771f94c6496dc84f3bd",
      "title": "Margherita Pizza",
      "restaurant": "6530ebbcc9e72013e5b65933",
      "rating": 4.5,
      "ratingCount": "150",
      "imageUrl": [
        "https://d326fntlu7tb1e.cloudfront.net/uploads/5c2a9ca8-eb07-400b-b8a6-2acfab2a9ee2-image001.webp",
        "https://d326fntlu7tb1e.cloudfront.net/uploads/5c2a9ca8-eb07-400b-b8a6-2acfab2a9ee2-image001.webp"
      ]
    },
    "additives": ["Extra Cheese", "Mushrooms"],
    "instructions": "",
    "totalPrice": 25,
    "quantity": 2,
    "__v": 0
  }
];

const profile = {
  "_id": "6537a4448cd1bd140ebddcee",
  "username": "Dbestech",
  "email": "db@king.com",
  "uid": "4NmOkCbvu7ToaBS9ZR1UVpv0G1g2",
  "address": [],
  "userType": "Vendor",
  "profile":
      "https://d326fntlu7tb1e.cloudfront.net/uploads/bdec9d7d-0544-4fc4-823d-3b898f6dbbbf-vinci_03.jpeg",
  "updatedAt": "2023-10-24T11:02:28.215Z"
};

const choicesList = [
  {
    "id": 1,
    "name": "Pick Up",
    "value": "pickup",
  },
  {
    "id": 2,
    "name": "4 Star",
    "value": "4star",
  },
  {
    "id": 3,
    "name": "3 Star",
    "value": "3star",
  },
  {
    "id": 4,
    "name": "Under 30 min",
    "value": "under30",
  },
  {
    "id": 5,
    "name": "Recommended",
    "value": "recommended",
  },
];
